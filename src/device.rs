...
use hidapi::HidApi;
use crate::protocol::{RazerReport, Command, NOSTORE, VARSTORE};
...
impl RazerDevice {
    ...
    pub fn open(path: &str) -> Result<Self> {
        let api = HidApi::new().context("Failed to initialize HID API")?;
        let handle = api.open_path(std::ffi::CString::new(path)?.as_c_str())
    ...
    fn send_command(&mut self, report: &RazerReport) -> Result<RazerReport> {
        ...
        self.handle.send_feature_report(&with_report_id)
        ...
        let len = self.handle.get_feature_report(&mut response)
    ...
    pub fn get_firmware_version(&mut self) -> Result<String> {
        ...
        tracing::debug!("Firmware response data[0..8]: {:02x?}", &response.data[0..8]);
